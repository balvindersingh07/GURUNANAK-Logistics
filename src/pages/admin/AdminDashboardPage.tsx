...
export default function AdminDashboardPage() {
  ...
  const { deliveries, drivers, vehicles, customers, notifications, getCustomer } = useApp();
  ...
  return (
    <div>
      <PageHeader
        ...
        description="System-wide management metrics across fleet, workforce, customers, and deliveries"
        actions={
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-4">
        <KPICard title="Total Deliveries" value={deliveries.length} icon={Package} accent={0} />
        <KPICard title="Active Deliveries" value={activeDeliveries} icon={TrendingUp} accent={3} />
        <KPICard title="Total Drivers" value={drivers.length} icon={Users} accent={2} />
        <KPICard title="Total Vehicles" value={vehicles.length} icon={Truck} accent={1} />
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 mb-6">
        <KPICard title="Total Customers" value={customers.length} icon={UserCircle} accent={4} />
        <KPICard title="Delivered / Completed" value={deliveredCount} icon={Package} accent={3} />
        <KPICard
          title="System Alerts"
          value={unreadAlerts}
          icon={Bell}
          accent={5}
          trend={unreadAlerts > 0 ? 'Unread notifications' : 'All clear'}
        />
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 mb-6">
        ...
        <Button variant="outline" className="justify-start" onClick={() => navigate('/fleet')}>
          <Truck size={18} /> Manage Fleet
        </Button>
        <Button variant="outline" className="justify-start" onClick={() => navigate('/customers')}>
          <UserCircle size={18} /> Manage Customers
        </Button>
        <Button variant="outline" className="justify-start" onClick={() => navigate('/routes')}>
        ...
        <Button variant="outline" className="justify-start" onClick={() => navigate('/deliveries')}>
          <Package size={18} /> View All Deliveries
        </Button>
      </div>
